import React, { useState } from 'react';
    import { Copy, RefreshCw, FileText, CheckCircle, Video, User, MessageSquare, Target, Newspaper, AlertTriangle, Sparkles, Sliders, Zap, Ghost, Eye, Handshake, Star } from 'lucide-react';
    ...
    [MENSAJE]: ${formData.mensajeClave}
      ...
      const generateScript = async () => {
        ...
        try {
          ...
          if (!response.ok) throw new Error('Error conectando con Make');
          
          const text = await response.text();
          
          if (text) setGeneratedScript(text);
          else throw new Error('Make no devolvió texto');

        } catch (err) {
          console.error(err);
          setError('Error generando el guion. Revisa tu escenario de Make.');
        } finally {
          setLoading(false);
        }
      };
